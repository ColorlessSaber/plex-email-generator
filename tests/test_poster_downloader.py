...
class TestPosterDownloader(TestCase):
    ...
    def test_successful_poster_downloads(self):
        ...
        assert Path(POSTERS_DIR / 'Crusher Joe OVA.jpg').exists(), 'failed to download Crusher Joe OVA poster'
        assert Path(POSTERS_DIR / 'Lost Universe.jpg').exists(), 'failed to download Lost Universe poster'
        assert Path(POSTERS_DIR / 'Spice & Wolf.jpg').exists(), 'failed to download Spice & Wolf poster'
    ...
    def test_bad_url_path(self):
        ...
        assert Path(POSTERS_DIR / 'Lost Universe.jpg').exists() == False, 'failed to handle bad URL path'
