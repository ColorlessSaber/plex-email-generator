...
def poster_downloader(posters_to_download: tuple[MediaItem, ...]) -> None:
    ...
    )  # create the data/posters folder if it does not exist
    ...
    for media_item in posters_to_download:
        if not (POSTERS_DIR / f"{media_item.title}.jpg").is_file():
            ...
            if response.status_code == 200:
                ...
                print(
                    f"failed to download {media_item.title}. Status code: {response.status_code}"
                )  # TODO have this error be logged
        else:
